...
for item in "$@"; do
    ...
    echo "Olaaaa"
    echo $nomeproj
    randomstring=$(cat /dev/urandom | env LC_CTYPE=C tr -dc 'a-zA-Z0-9' | fold -w 7 | head -n 1)
    echo $randomstring
    chave="ch$nomecoop-$randomstring"
    sed -i '' -e "s/CHAVE_APPKEY/$chave/g" $nomeproj/Miscelâneas/dados.d
    sed -i '' -e "s/NOME_PROJETO/$nomecoop/g" $nomeproj/Miscelâneas/dados.d
