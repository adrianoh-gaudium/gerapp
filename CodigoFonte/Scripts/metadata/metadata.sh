...
play_store_json="${17}"  # Json p/ upload na play store
itunes_team_name="${18}"	#Team_name do iTunes Connect (App Store Connect)

# custom_json=$(dirname "${play_store_json}")/"${nomecoop}".json
# if test -f "$custom_json"; then
# 	play_store_json="${custom_json}"
# fi
...
backup="$nomedir"/"$diretorio_dados"/Metadados/backup

mkdir -p "$backup"/android "$backup"/taxista "$backup"/ios

mv  "$nomedir"/"$diretorio_dados"/Metadados/android/metadata/"$idioma"/images/phoneScreenshots/* "$backup"/android
mv  "$nomedir"/"$diretorio_dados"/Metadados/taxista/metadata/"$idioma"/images/phoneScreenshots/* "$backup"/taxista
mv  "$nomedir"/"$diretorio_dados"/Metadados/ios/screenshots/"$idioma"/* "$backup"/ios
mv  "$nomedir"/"$diretorio_dados"/Metadados/ios/metadata/"$idioma"/* "$backup"/ios
...
cp "$pasta_original"/metadata/base/ios/"$tipo"/ratings_config.json "$nomedir"/"$diretorio_dados"/Metadados/ios
...
cp -v "$nomedir"/"$diretorio_imagens"/Telas/Android/*.png "$nomedir"/"$diretorio_dados"/Metadados/android/metadata/"$idioma"/images/phoneScreenshots/
...
if ( [ $target = "ios" ] || [ $target = "both" ] ) && ( [ $typeTarget = "both" ] || [ $typeTarget = "psg" ] ); then
	...
	DELIVER_SKIP_WAIT_FOR_SCREENSHOT_PROCESSING=true

	cd "$nomedir"/"$diretorio_dados"/Metadados/ios

 	if [ -z "$itunes_team_name" ]; then
		echo fastlane deliver -a "$bundleid" --force --team_name "Gaudium Internet"
		fastlane deliver -a "$bundleid" --force --team_name "Gaudium Internet"
	else
		echo fastlane deliver -a "$bundleid" --force --team_name "$itunes_team_name"
		fastlane deliver -a "$bundleid" --force --team_name "$itunes_team_name"
	fi
...
#	supply --skip_upload_apk -j /Users/integrador/ProjetosAndamento/Proj076-TaxiMachine/Miscelâneas/"$prefixo"_secret.json -p br.com."$nomecoop".passenger."$bundletipo"machine
	echo fastlane supply --skip_upload_apk -j "$play_store_json" -p br.com."$nomecoop".passenger."$bundletipo"machine
	fastlane supply --skip_upload_apk --skip_upload_changelogs -j "$play_store_json" -p br.com."$nomecoop".passenger."$bundletipo"machine
...
#	supply --skip_upload_apk -j /Users/integrador/ProjetosAndamento/Proj076-TaxiMachine/Miscelâneas/"$prefixo"_secret.json -p br.com."$nomecoop".taxi."$bundletipo"machine
	echo fastlane supply --skip_upload_apk -j "$play_store_json" -p br.com."$nomecoop".taxi."$bundletipo"machine
	fastlane supply --skip_upload_apk --skip_upload_changelogs -j "$play_store_json" -p br.com."$nomecoop".taxi."$bundletipo"machine
# Fim - Horácio - 16/09/2018
